...
export default abstract class Potter<TRepository, TModel, TState extends PotterState<TRepository, TModel>>
  implements IPotter<TRepository, TModel> {
  ...
  constructor(repository: TRepository, model: TModel, state: TState) {
    ...
    this.state = state;
    this.state.initialize(this.context, this);
    this.shuttler = new Shuttler<PotterContext<TRepository, TModel>>(this.context);
  }
